...
test("projects append-only artifact progress entries into one terminal card with aligned ids", () => {
  const bundle = (status, revision, artifacts = []) => ({
    schema: "pi.artifact-bundle/v1",
    provider: "comfyui",
    runId: "prompt-progress",
    revision,
    status,
    artifacts,
  });
  const entries = [
    userEntry("u1", null, "render"),
    {
      type: "custom",
      id: "p1",
      parentId: "u1",
      timestamp: "2026-01-01T00:00:01.000Z",
      customType: "pi.artifact-progress",
      data: bundle("queued", 1),
    },
    {
      type: "custom",
      id: "p2",
    ...
    },
    {
      ...
      content: "done",
      display: true,
      details: bundle("succeeded", 3, [{ kind: "image", path: "C:\\renders\\done.png" }]),
    },
  ];

  const context = buildSessionContext(entries);
  assert.deepEqual(context.entryIds, ["u1", "done"]);
  assert.equal(context.messages.length, context.entryIds.length);
  assert.equal(context.messages[1].details.status, "succeeded");
});
